...
- `photo-041-uploaded.png`

网页会按照片编号和 `script.js` 里的 `CONFIG.sketches.assignments` 自动寻找对应的小卡片。后续通过上传流程新增的小卡片由用户自己生成并上传，文件名会保存为 `photo-xxx-uploaded.png`。当前小卡片区不再分类，会把所有小卡片放在一条自动滚动轨道里展示。
...
2. 把多张照片拖进表单并提交。
3. GitHub Action 自动下载原照片和用户上传的小卡片，并完成归类配对。
